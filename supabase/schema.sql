...
create policy "debrief_replies_select_all" on public.debrief_replies for select using (auth.uid() is not null);
create policy "debrief_replies_insert" on public.debrief_replies for insert
  with check (
    user_id = auth.uid()
    and (
      exists (select 1 from public.profiles where id = auth.uid() and role in ('patron', 'responsable'))
      or exists (select 1 from public.debriefs d where d.id = debrief_id and d.user_id = auth.uid())
    )
  );
create policy "debrief_replies_delete_own" on public.debrief_replies for delete
  using (user_id = auth.uid());
